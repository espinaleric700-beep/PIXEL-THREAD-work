...
# --- CSS AVANZADO: EL "CORAZÓN" DEL REDISEÑO ---
st.markdown("""
<style>
    :root {
        --primary: #00ffcc;
...
if "modo_vista" not in st.session_state: st.session_state.modo_vista = params.get("seccion", "Cliente")
if "user" not in st.session_state: st.session_state.user = params.get("user", "")
...
with col_nav1:
    if st.button("👤 PANEL CLIENTE"): actualizar_url("Cliente", st.session_state.user)
with col_nav2:
    if st.button("🛠️ PANEL ADMIN"): actualizar_url("Admin", st.session_state.user)

st.markdown("<br>", unsafe_allow_html=True)

if st.session_state.modo_vista == "Cliente":
    # Lógica de cliente...
    user_input = st.text_input("Identificación de Usuario:", value=st.session_state.user)
    if user_input != st.session_state.user: actualizar_url("Cliente", user_input)
